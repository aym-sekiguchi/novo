		...
		<>
			<Screen avatar={avatar} breadcrumb={[{ href: '', label: 'Home' }]} isPublic={isPublic} projectName={name} role={role} username={username}>
				<div className="grid gap-6 sm:grid-cols-[repeat(auto-fill,minmax(20rem,1fr))]">
					<Link
						className={cn(
							'relative min-h-[11.25rem] transition hover:brightness-90',
							!propertyIsPublic && role === 'editor' && 'pointer-events-none',
						)}
						href={`/${username}/property`}
					>
						{!propertyIsPublic && role === 'editor' && (
							<div className="bg-background/75 absolute inset-0 flex size-full flex-col items-center justify-center">
